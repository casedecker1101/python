...
def get_drink_order():
    ...
    while choice == False:
        ...
        # locate the price of the drink based 
        if drink_size is not None:
            drink_price = DRINK_PRICES[DRINK_SIZES.index(drink_size)]
        d = drink(drink_name,drink_size,drink_price)
        return d

def get_side_order():
        print("Thexe are the available sides: ")
        print(SIDES)
        choice = False
        side_name = None
        while choice == False:
...
def get_combo_order():
    ...
    b = get_burger_order()
    print("Now, let's order the drink for your comb o.")
    d = get_drink_order()
...
def order_many():
    print("Welcome to Mary Janes's Burger Shop!")
